 ...
 *
 * `@noble/hashes`'s `hkdf` (v2) requires `info` as bytes rather than the
 * string `@noble/hashes` v1 accepted, so the fixed ASCII `info` strings are
 * UTF-8 encoded here, once per call — the encoded bytes are identical to the
 * ASCII source either way, so this changes no derived value.
 */
function hkdfKey(ikm: Uint8Array, info: string): Uint8Array {
  return hkdf(sha256, ikm, undefined, UTF8_ENCODER.encode(info), 32);
}
 ...
 *
 * The hash is the same one `wire/condition.ts` already uses; what ADR 0019
 ...
 * `wire/condition.ts` for that rather than a second copy of it here.
 */
