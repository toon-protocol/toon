...
3. Push, then CONFIRM the remote branch actually advanced — a push can fail
   without an obvious error:

   `git push origin {{BRANCH}}`
   `git ls-remote --heads origin {{BRANCH}}`

   Compare the sha `git ls-remote` prints against your local tip
   (`git rev-parse {{BRANCH}}`). If they do NOT match, the push FAILED. Do
   **not** output `<promise>COMPLETE</promise>` — print the push error and stop.
   The runner verifies the remote tip advanced and will fail the job if it did
   not.
...
- Only output `<promise>COMPLETE</promise>` once the remote tip for `{{BRANCH}}`
  matches your local tip. A failed push is a failure, not a COMPLETE.

Once the push is confirmed to have advanced the remote branch, output
